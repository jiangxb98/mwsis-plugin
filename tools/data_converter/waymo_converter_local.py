...
class Waymo2KITTI(object):
    ...
    def save_lidar(self, frame, file_idx, frame_idx, frame_info, frame_infos, sample_idx):
        ...
        point_cloud = np.column_stack(
            (points, intensity, elongation, range_dist, 
             ri_index, sensor_index, mask_indices, 
             cp_points[:, 0] - 1, cp_points[:, 3] - 1,
             cp_points[:, 1], cp_points[:, 4], 
             cp_points[:, 2], cp_points[:, 5])).astype(np.float32)
