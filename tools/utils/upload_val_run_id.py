...
class UploadLabel:
    ...
    def upload_one(self, ind):
        ...
        pts_path = '{}/{}/{}'.format(pts_path[0],'mwsis_ccl',pts_path[2])

        results_dtype = [('norm_box_1', 'i4'), ('norm_box_2', 'i4'),('assi_box_1', 'i4'), ('assi_box_2', 'i4'), ('run_id', 'i4'), ('mask_flag', 'i4')]
        upload_results = np.empty(len(points), dtype=results_dtype)
        upload_results['norm_box_1'] = points[:, -4]
        upload_results['assi_box_1'] = points[:, -2]
        upload_results['norm_box_2'] = points[:, -3]
        upload_results['assi_box_2'] = points[:, -1]
        upload_results['run_id'] = run_id
        upload_results['mask_flag'] = points[:, 17]
    ...
    def query_index(self, filter_sem='semseg_info', test_model=True):
        mongo_client = pymongo.MongoClient(**self.mongo_cfg)[self.database]
        while True:
            try:
