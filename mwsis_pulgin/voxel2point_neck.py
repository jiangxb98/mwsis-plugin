    ...
    def forward(self, points, pts_coors, voxel_feats, voxel2point_inds, voxel_padding=-1):
        ...
        if self.with_xyz:
            ...
            if self.training and not self.normalize_local_xyz:
                ...
                if (local_xyz.abs() < voxel_size / 2 + 1e-3).all() is False:
                    print("\n Holds in training. However, in test, this is not always True because of lack of point range clip")
                    print('\n over point range nums{}'.format((local_xyz.abs() >= voxel_size / 2 + 1e-3).sum()))
